<html>
<title>Professors management - Arellano University Subject Advising - AUSMS </title>
...
Professors management - Arellano University Subject Advising - AUSMS 
</header>
...
<body>
    ...
    <?php endif; ?>
     ...
     <?php if ($_SESSION['usertype'] == "ADMINISTRATOR" || $_SESSION['usertype'] == "DEAN") : ?>
       <a href="professors-management.php" style = "background-color: white; color: black;">Professors Management</a>
    <?php endif; ?>
...
<div class="container">
	<form action = "<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method = "POST">
		...
		Search: <input type="text" name="txtsearch" placeholder="Enter ID#, Lastname, Firstname or Subjects" style="width: 50%;" >
        <button name="btnsearch" class="btn btn-info">
        	<i class="fas fa-search"></i> Search
        </button>

		<a href="create-prof.php" style="float: right;" class = 'btn btn-primary' data-toggle="modal" data-target="#modalCreate"> Create new professor account</a>
...
function buildtable($result) {
	if(mysqli_num_rows($result) > 0){
		...
		echo "<tr>";
		echo "<th>Employee #</th><th>Last name</th><th>First name</th><th>Second name</th><th>Middle name</th><th>Faculty</th><th>Subjects</th><th>Action</th>";
		echo "</tr>";
		// display
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			echo "<td>" . $row['Psecondname'] . "</td>";
			echo "<td>" . $row['Pmiddlename'] . "</td>";
			echo "<td>" . $row['faculty'] . "</td>";

			echo "<td>";
			echo "<a href = 'assigned-subjects.php?ID_number=" . $row['ID_number']. "' class = 'btn btn-default'>Subjects</a>";
			echo "</td>";

			echo "<td>";
			echo "<a href = 'update-prof.php?ID_number=" . $row['ID_number']. "' class = 'btn btn-success' data-toggle='modal' data-target='#modalUpdate' >Update</a>";
			echo "</td>";
...
if (isset($_POST['btnsearch'])) {
	$sql = "SELECT * FROM  tblprofessors WHERE ID_number LIKE ? OR Pfirstname LIKE ? OR Plastname LIKE ? OR subject_1 LIKE ? OR subject_2 LIKE ? OR subject_3 LIKE ? ORDER BY ID_number";
	...
	$sql = "SELECT * FROM tblprofessors ORDER BY ID_number";
	if ($stmt = mysqli_prepare($link, $sql)) {
		if (mysqli_stmt_execute($stmt)) {
			$result = mysqli_stmt_get_result($stmt);
			buildtable($result);
