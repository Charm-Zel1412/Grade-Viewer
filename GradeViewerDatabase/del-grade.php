		...
		if (mysqli_stmt_execute($stmt)) {
			...
			if ($stmt = mysqli_prepare($link, $sql)) {
				...
				if (mysqli_stmt_execute($stmt)) {
					...
					echo "<script>location = 'subjects-grade.php?studentnumber=" . $_POST['txtstudentnum'] . "';</script>";
					exit();
				} else {
...
<body>
	<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
		...
		<center>
			...
			<?php echo "<a href='subjects-grade.php?studentnumber=" . htmlspecialchars($_GET['studentnumber']) . "' class='btn btn-default'>No</a>"; ?>
	    </center>
	</form>
</body>
</html>
